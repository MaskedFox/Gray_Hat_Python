...
def printf_randomizer(dbg):
    ...
    # When we sue read_process_memory, it returns a packed binary
    # String. We must first unpack it before we can use it further.
    counter = struct.unpack("L", counter)[0]
    print(f"Counter: {}").format(counter)

    # Generate a random number and pack it into binary format
    # so that is written correctly back into the process
    random_counter = random.randint(1,100)
    random_counter = struct.pack("L", random_counter)[0]

    # Now swap in our randomnumber and resume the process
